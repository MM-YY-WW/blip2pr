...
# ----------------------------------------------GenePT-------------------------------------
class GenePT_W(torch.nn.Module):
    def __init__ (self, num_class, text_embedding):
        super(GenePT_W, self).__init__()
        self.text_emb_dim = 1536
        self.num_of_calss = num_class
        self.Linear = nn.Linear(self.text_emb_dim,self.num_of_calss)
    ...
    def forward(self, *argv):
        if len(argv) == 2:
            X, text_embedding = argv
        elif len(argv) ==1:
            X = argv[0]
        if len(X.shape) == 3:
            X= torch.sum(X, dim=2)
        out = torch.matmul(X, text_embedding)
        pred = self.Linear(out)
        #out = nn.Softmax(out)
        return out.detach(), pred
    
class GenePT_Exponly(torch.nn.Module):
    def __init__ (self, num_class, num_gene):
        super(GenePT_Exponly, self).__init__()
        self.num_of_calss = num_class
        self.num_of_gene = num_gene
        self.Linear = nn.Linear(self.num_of_gene,self.num_of_calss)
    ...
    def forward(self, *argv):
        ...
        if len(X.shape) == 3:
            X= torch.sum(X, dim=2)
        pred = self.Linear(X)
        return X.detach(), pred
    
class GenePT_attention(torch.nn.Module):
    def __init__ (self, num_class, hidden_k_size, num_gene, text_embedding):
        super(GenePT_attention, self).__init__()
        #           ________________                     _______________                    ________________                    ________________
        #           |               |                    |              |                   |               |                   |               |                                                                                                         
        #batch_size |               |   X   3000(gene)   |              |       X       ?   |               |       X    3000   |               |
        #           |_______________|                    |______________|                   |_______________|                   |_______________|
        #                3000(gene)                             ?                                  3000                                 1536
        #                  Exp                                   K                             K.transpose                        text_embedding
        self.hidden_k_size = hidden_k_size
        self.text_emb_dim = 1536
        self.num_of_class = num_class
        self.num_of_gene = num_gene
        self.output = nn.Linear(self.text_emb_dim, self.num_of_class)
        self.k = nn.Parameter(torch.randn(self.num_of_gene, hidden_k_size))
        self.kt = nn.Parameter(torch.randn(hidden_k_size, self.num_of_gene))
        self.text_embedding = text_embedding
        # self.k.weight.data.uniform_(-1,1)
        # self.kt.weight.data.uniform_(-1,1)
    def from_pretrained(self,model_file):
        a=1
    def forward(self, *argv):
        if len(argv) == 2:
            X, text_embedding = argv # X [32, 3000] text [3000, 1536]
        elif len(argv) == 1:
            X = argv[0]
        if len(X.shape) == 3:
            X= torch.sum(X, dim=2)
        out = torch.matmul(X, self.k) #[32,512]
        out = torch.matmul(out, self.kt) #[32, 3000]
        out = torch.matmul(out, text_embedding) #[32, 1536]
        pred = self.output(out) #[32, class_num]
        return out.detach(), pred
...
class Gene2VecPositionalEmbedding(nn.Module):
    def __init__(self, dim, max_seq_len):
        super().__init__()
        gene2vec_weight = np.load('./pretrained_model/scBERT/gene2vec_16906.npy')
    ...
    def forward(self, x, return_encodings = False, output_attentions = False, **kwargs):
        ...
        if output_attentions:
            ...
            if exists(self.to_out):
                #
                x = self.to_out(x)
                return x #[batch_size, 11]

            return x @ self.token_emb.weight.t() #[3, 16907, 200] @ [200, 7] = [3, 16907, 7]




#make the final prediction?
class scBERT_Identity(torch.nn.Module):
    def __init__(self, dropout = 0., h_dim = 100, out_dim = 10, num_gene=16907):
        super(scBERT_Identity, self).__init__()
        self.conv1 = nn.Conv2d(1, 1, (1, 200))
        self.act = nn.ReLU()
        self.fc1 = nn.Linear(in_features=num_gene, out_features=512, bias=True)
        self.act1 = nn.ReLU()
